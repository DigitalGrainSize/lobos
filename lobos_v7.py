...
Builder.load_file('lobos_test.kv')
...
class CameraWidget(BoxLayout):
    ...
    def TakePicture(self, fig, mode):#*args):
        ...
        if mode==1:
            ...
            filename = 'st'+self.txt_inpt.text+'_cobblegravel_'+now+'_'+self.e_txt.text+'_'+self.n_txt.text+'.png'
        elif mode==4:
            ...
            filename = 'st'+self.txt_inpt.text+'_transition_'+now+'_'+self.e_txt.text+'_'+self.n_txt.text+'.png'
        elif mode==8:
            filename = 'st'+self.txt_inpt.text+'_cobble_'+now+'_'+self.e_txt.text+'_'+self.n_txt.text+'.png'
        elif mode==9:
            filename = 'st'+self.txt_inpt.text+'_boulder_'+now+'_'+self.e_txt.text+'_'+self.n_txt.text+'.png'
        ...
        if mode==1:
            ...
            subprocess.Popen("python resize_n_move.py -i "+filename+" -o cobblegravelimages", shell=True)
            self.textinput.text += 'Cobble/Grvl image @ '+time.asctime().split(' ')[3]+'\n'
        elif mode==4:
            ...
            subprocess.Popen("python resize_n_move.py -i "+filename+" -o transitionimages", shell=True)
            self.textinput.text += 'Transition image @ '+time.asctime().split(' ')[3]+'\n'
        elif mode==8:
            subprocess.Popen("python resize_n_move.py -i "+filename+" -o cobbleimages", shell=True)
            self.textinput.text += 'Cobble image @ '+time.asctime().split(' ')[3]+'\n'
        elif mode==9:
            subprocess.Popen("python resize_n_move.py -i "+filename+" -o boulderimages", shell=True)
            self.textinput.text += 'Boulder image @ '+time.asctime().split(' ')[3]+'\n'
        ...
        if SHOWMAP:
            if mode==1:
                ...
                fig.gca().plot(float(self.e_txt.text),float(self.n_txt.text),'s', color=(0.0, 0.0, 0.5))
            elif mode==8:
                fig.gca().plot(float(self.e_txt.text),float(self.n_txt.text),'s', color=(0.5, 0.0, 0.0))
            elif mode==9:
                    fig.gca().plot(float(self.e_txt.text),float(self.n_txt.text),'s', color=(0.5, 0.0, 0.0))
            #elif mode==9:
            #    fig.gca().plot(float(self.e_txt.text),float(self.n_txt.text),'s', color=(0.0, 0.0, 0.5))
    ...
    #=========================  RGB VALUES ALL SET TO BLACK BY TUSSO
    def station_up(self):
        '''
        increment station 1
        '''
        ...
        with open('lobos.cfg','rb') as oldfile, open('lobos_new.cfg','wb') as newfile:
           for line in oldfile:
              ...
              if counter==countmax:
                 #newfile.write("            text: '"+st+"'\n")
                 newfile.write('station_start = '+self.txt_inpt.text+'\n')
                 print "spurs"+self.txt_inpt.text
              else:
                 newfile.write(line)
        mv('lobos_new.cfg','lobos.cfg')
...
class Eyeball_DAQApp(App):
    ...
    font_sizezero = NumericProperty(0)
    ...
    def build(self):
        ...
        self.textinput = Log(text='Data Acquisition Log\n', size_hint = (0.05, .99), markup=True, font_size='11sp')
        self.textinput.counter=0
        ...
        Clock.schedule_interval(self._update_pos, 1.6) #update position
        # Clock.schedule_interval(self._update_dep, 1) #update depth
...
if __name__ == '__main__':
    ...
    try:
       ...
       os.mkdir('transitionimages')
       os.mkdir('cobbleimages')
       os.mkdir('boulderimages')
       os.mkdir('cobblegravelimages')
    except:
       pass
